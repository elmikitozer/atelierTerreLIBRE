...
export default async function TeamBuildingSection() {
  ...
  return (
    <section id="team-building" className="px-5 md:px-12 py-12 md:py-16 bg-ink border-b border-yellow/10">
      ...
      >
        <p className="font-mono text-[10px] md:text-[11px] tracking-[0.22em] uppercase text-yellow/60 mb-3">
          Pour les entreprises
        </p>
        <h2 className="font-archivo-black text-[28px] md:text-[36px] leading-[0.95] text-yellow">
        ...
        <p className="font-mono text-[10px] md:text-[11px] tracking-[0.22em] uppercase text-yellow/60 mt-6 mb-3">
          Cinq objectifs
        </p>
        <ul className="flex flex-col gap-2">
          {OBJECTIFS.map((obj, i) => (
            <li key={i} className="font-news text-[14px] md:text-[15px] text-cream/85 leading-snug border-b border-yellow/10 pb-2">
              {obj}
            </li>
          ))}
        </ul>
      ...
      {/* Mention tarif + CTA */}
      <div className="mt-10 border-t border-yellow/10 pt-8 md:flex md:items-center md:justify-between md:gap-10">
        ...
        <Link
          href="/contact"
          className="mt-5 md:mt-0 shrink-0 block md:inline-block bg-yellow text-ink rounded-full text-center font-manrope font-semibold text-[13px] md:text-[14px] py-3.5 px-8 hover:opacity-80 transition-opacity"
        >
          Demander un devis →
        </Link>
      </div>
